...
import { useEditChatDescription, useMeetingContext } from '~/lib/hooks';
import { forwardRef, type ForwardedRef, useCallback, useMemo } from 'react';
...
export function HistoryItem({
  ...
  // Generate the appropriate chat link based on context
  const chatLink = useMemo(() => {
    // If in a meeting, use query parameter to stay in meeting context
    if (isMeeting && basePath) {
      return `${basePath}?chat=${item.urlId}`;
    }
    // Otherwise, use normal chat route
    return `/chat/${item.urlId}`;
  }, [isMeeting, basePath, item.urlId]);
