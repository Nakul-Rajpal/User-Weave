...
export abstract class BaseProvider implements ProviderInfo {
  ...
  getProviderBaseUrlAndKey(options: {
    ...
    console.log(`🔍 DEBUG [${this.name}] - Looking for API key with token key: ${apiTokenKey}`);
    console.log(`🔍 DEBUG [${this.name}] - apiKeys?.['${this.name}']:`, !!apiKeys?.[this.name]);
    console.log(`🔍 DEBUG [${this.name}] - serverEnv?.['${apiTokenKey}']:`, !!serverEnv?.[apiTokenKey]);
    console.log(`🔍 DEBUG [${this.name}] - process?.env?.['${apiTokenKey}']:`, !!(typeof process !== 'undefined' && process?.env?.[apiTokenKey]));
    console.log(`🔍 DEBUG [${this.name}] - manager.env?.['${apiTokenKey}']:`, !!manager.env?.[apiTokenKey]);
    // ========== END DEBUG ==========

    const apiKey =
      apiKeys?.[this.name] || serverEnv?.[apiTokenKey] || process?.env?.[apiTokenKey] || manager.env?.[apiTokenKey];

    console.log(`🔍 DEBUG [${this.name}] - Final API key found:`, !!apiKey);
