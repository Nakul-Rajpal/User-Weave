...
async function chatAction({ context, request }: ActionFunctionArgs) {
  ...
  const { messages, files, promptId, contextOptimization, chatMode, designScheme, maxLLMSteps } =
    await request.json<{
      messages: Messages;
      files: any;
      promptId?: string;
  ...
  // DEBUG: Log API keys availability (without exposing actual keys)
  logger.info('🔑 [API_KEYS_CHECK]', {
  ...
  try {
    ...
    const dataStream = createDataStream({
      async execute(dataStream) {
        ...
        const options: StreamingOptions = {
          ...
          onStepFinish: ({ toolCalls }) => {
            // Tool calls are no longer processed since MCP is removed
            toolCalls.forEach((toolCall) => {
              // MCP tool processing removed
            });
          },
        ...
        // DEBUG: Log before streamText call
        const lastMessage = messages[messages.length - 1];
        const { model, provider } = extractPropertiesFromMessage(lastMessage);
        logger.info('🚀 [STREAM_TEXT_CALL]', {
        ...
        logger.info('✅ [STREAM_TEXT_SUCCESS] StreamText call succeeded');
        ...
        (async () => {
          for await (const part of result.fullStream) {
            ...
            if (part.type === 'error') {
              const error: any = part.error;
              logger.error('❌ [STREAMING_ERROR] Error in fullStream:');
      ...
      onError: (error: any) => {
        ...
        logger.error('❌ [ON_ERROR_HANDLER] Error occurred:');
